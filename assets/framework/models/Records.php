...
class Records extends DB\SQL\Mapper{
	...
	public function countRecordsByDomainID($domainid) {
		return count($this->load(array('id=?', $id)));
	}

	public function getDomainRecords($domainid){
		$this->load(array('type!=? AND domain_id=?','SOA',$domainid),array(
	...
	public function deleteHost($recordid,$recordtype,$recordname,$recordcontent,$domainid) {
		$this->load(array('id=? AND type=? AND name=? AND content=? AND domain_id=?',$recordid,$recordtype,$recordname,$recordcontent,$domainid));
		if($this->dry()) {
			return "error";	
		} else {
			$this->erase();
	...
	public function updateSOA($domainId,$soaprimary,$soaemail,$soaserial,$soarefresh,$soaretry,$soaexpire,$soattl) {
		$soa = new SOA($this->db);
		$this->load(array('type=? AND domain_id=?','SOA',$domainId));
